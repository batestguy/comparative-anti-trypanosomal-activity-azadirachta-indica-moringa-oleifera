...
- `analysis/R/02_analysis.R` runs the block-level analysis → `analysis/results/tables/*.csv` + `figures/*.png` (16 + 7). `analysis/plans/analysis-plan.md` (+ `.Rmd`) was the spec. `render_docx.R` reads the `.md`, `render_rmd.R` reads the `.Rmd` — don't swap inputs.

## Reports (Word: Times New Roman + APA tables)

- `quarto render analysis/reports/report.qmd` writes `report.docx` + `report.html` next to the `.qmd` — do NOT pass `--output-dir` (it nests outputs one level too deep). Docx styling comes from `analysis/reports/reference-doc.docx`.
- `Rscript analysis/R/render_methods_docx.R` writes `analysis/reports/objectives-methods.docx` using `analysis/R/template-tnr.docx` + flextable `theme_apa()`.
- Both templates are generated by `python3 analysis/R/make_templates.py` (pandoc default + Times/APA patches) — regenerate rather than hand-editing the `.docx` templates.
